...
const SkillsForm = ({ data, onChange }) => {
  ...
  return (
    <div className="space-y-4">
      <div>
        <h3 className="flex item-center gap-2 text-lg font-semibold text-gray-900">
          {" "}
          Skills{" "}
        </h3>
            ...
            <div className="flex flex-wrap gap-2">
                {
                    data.map((skill, index) => (
                        <span key={index} className="flex items-center gap-1 px-3 py-1 text-sm bg-blue-600
                text-blue-800 rounded-full">
                            {skill}
                            <button 
                            onClick={() => removeSkill(index)} className="ml-1 hover:bg-blue-200 rounded-full p-0.5 transition-colors">
                                <X className="w-3 h-3" />
                            </button>
                        </span>
                    ))
